...
// หน้ารวมเกม What Words / Word Shuffle ของทีมงาน — โครงเดียวกับ PuzzleManager ทุกอย่าง
// game = 'words' | 'shuffle' (ดู lib/wordGames.js) — หน้าเดียวกัน ต่างแค่ชุด/ห้องที่กรองกับข้อความ
//   1. ห้องที่เปิดอยู่ — เข้าไปคุมต่อ หรือปิดทิ้ง
//   2. คลังชุดคำ — สร้างห้องจากชุดไหนก็ได้ (เล่นเดี่ยว/เป็นทีม)
//      ชุดแบบ "ไม่ต้องตอบ" (answer_mode = 'none') ไม่มีคะแนน จึงไม่มีตัวเลือกทีม
//
// ทุก query ต้องกรอง game_kind ของเกมนี้ เพราะทุกเกมใช้ตารางร่วมกัน
// ถ้าลืม ชุดของเกมอื่นจะมาโผล่ที่นี่แล้วกดสร้างห้องได้ทั้งที่หน้าจอคนละแบบ
...
export default function WordsManager({ game: gameKey = 'words' }) {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: roomRows }, { data: setRows }, { data: busRows }] = await Promise.all([
      ...
      supabase
        .from('quiz_sets')
        .select('id, title, description, default_time_limit, answer_mode')
        .eq('org_id', orgId)
        .eq('game_kind', G.kind)
  ...
  async function handleCreateRoom(set) {
    setError('')
    const watchOnly = set.answer_mode === 'none'
    try {
      // game_kind / stage_theme ถูกคัดจากชุดฝั่ง DB ไม่ใช่ส่งมาจากที่นี่
      // (ดู quiz_start_session ใน 20260911b_what_words.sql)
      const row = await startSession({
        ...
        // ชุดไม่ต้องตอบ = ไม่มีใครได้คะแนน ห้องทีมจะมีแต่กระดานศูนย์ทั้งแถบ
        teamMode: watchOnly ? false : form.teamMode,
        teamSizeLimit: watchOnly ? 0 : form.teamSizeLimit,
      })
      ...
      if (!watchOnly && form.teamMode && row?.session_id) {
        await setRoomSolveLimit(row.session_id, Number(form.solveLimit), row.token)
      }
      navigate(`${G.base}/host/${row.session_id}`)
    } catch (err) {
  ...
  async function handleNewSet() {
    const title = (newSetName ?? '').trim() || tw('manager.newSetName')
    setError('')
    const { data, error: err } = await supabase
      ...
      .insert({
        ...
        title,
        game_kind: G.kind,
        // คิดคำ + พิมพ์ไทยบนรถที่โยก เวลาเท่าปริศนาใบ้คำ
        ...
        solve_limit: 1,
        // วิธีตอบเริ่มที่พิมพ์ตอบในมือถือ — เปลี่ยนเป็น "ไม่ต้องตอบ" ได้ใน Builder (เหมือนเปิดแผ่นป้าย)
        answer_mode: 'type',
    ...
    setNewSetName(null)
    navigate(`${G.base}/builder/${data.id}`)
  }

  return (
    <div className="min-h-screen bg-surface-muted">
      <StaffHeader icon={G.icon} title={tw('title')} subtitle={tw('manager.subtitle')} />
      ...
      <div className="mx-auto max-w-md space-y-5 p-4">
        ...
        {/* ── ห้องที่เปิดอยู่ ─────────────────────────────────── */}
        <section>
          <h2 className="mb-2 text-sm font-bold text-ink-muted">{tw('manager.openRooms')}</h2>
          {loading ? (
              ...
              {rooms.map((room) => (
                ...
                >
                  ...
                  <p className="mt-0.5 text-xs text-ink-muted">
                    ...
                    {room.team_mode ? ` · ${tw('manager.teamBadge')}` : ''}
                  </p>
                  <div className="mt-3 flex gap-2">
                    <Button
                      fullWidth={false}
                      className="px-3 py-2 text-sm"
                      onClick={() => navigate(`${G.base}/host/${room.id}`)}
                    >
                      {tw('manager.enterRoom')}
                    </Button>
                    <Button variant="ghost" fullWidth={false} className="px-3 py-2 text-sm" onClick={() => handleClose(room)}>
                      {tw('manager.closeRoom')}
                    </Button>
                  </div>
                </li>
              ))}
        ...
        {/* ── คลังชุดปริศนา ───────────────────────────────────── */}
        <section>
          <h2 className="mb-2 text-sm font-bold text-ink-muted">{tw('manager.library')}</h2>
                  ...
                  <div className="mt-3 space-y-2">
                    ...
                    {buses.length > 0 && (
                      <select
                        ...
                        className="w-full rounded-xl border border-line bg-surface px-3 py-2 text-sm"
                      >
                        <option value="">{tw('manager.allBuses')}</option>
                        {buses.map((b) => (
                    ...
                    {/* เล่นเป็นทีม — ลูกทัวร์ตั้งทีมกันเองในห้องรอ (ของกลางชุดเดียวกับควิซ/แผ่นป้าย)
                        ...
                        เมื่อเป็นเกม puzzle/tiles/words) */}
                    {set.answer_mode !== 'none' && (
                      <label className="flex items-center gap-2 rounded-xl bg-neutral-bg px-3 py-2 text-sm">
                        <input
                          ...
                          className="h-4 w-4"
                        />
                        <span className="font-semibold text-ink">{tw('manager.teamMode')}</span>
                      </label>
                    )}

                    {set.answer_mode === 'none' && (
                      <p className="text-xs text-ink-faint">{tw('manager.watchOnlyHint')}</p>
                    )}

                    {set.answer_mode !== 'none' && form.teamMode && (
                      <>
                        <select
                          ...
                          className="w-full rounded-xl border border-line bg-surface px-3 py-2 text-sm"
                        >
                          <option value={0}>{tw('manager.teamSizeFree')}</option>
                          {[2, 3, 4, 5, 6, 8].map((n) => (
                            <option key={n} value={n}>
                              {tw('manager.teamSize')} {n}
                            </option>
                          ))}
                        </select>

                        <label className="block pt-1 text-xs font-bold text-ink" htmlFor={`solve-limit-${set.id}`}>
                          {tw('manager.solveLimit')}
                        </label>
                        <select
                        ...
                        >
                          {SOLVE_LIMIT_OPTIONS.map((n) => (
                            <option key={n} value={String(n)}>
                              {tw('manager.solveLimitN', { n })}
                            </option>
                          ))}
                          <option value="0">{tw('manager.solveLimitOff')}</option>
                        </select>
                        <p className="text-xs text-ink-faint">{tw('manager.solveLimitHint')}</p>
                        <p className="text-xs text-ink-faint">{tw('manager.teamHint')}</p>
                      </>
                    )}

                    <div className="flex gap-2">
                      <Button fullWidth={false} className="px-3 py-2 text-sm" onClick={() => handleCreateRoom(set)}>
                        {tw('manager.createRoom')}
                      </Button>
          ...
          {newSetName === null ? (
            ...
            <div className="mt-3 space-y-2 rounded-2xl border border-line bg-surface p-4">
              <input
                ...
                placeholder={tw('manager.newSetName')}
                className="w-full rounded-xl border border-line bg-surface px-3 py-2 text-sm"
              />
              <div className="flex gap-2">
