...
3. ~~วางไฟล์ฟอนต์ 4 ไฟล์ตาม `public/fonts/README.md`~~ ✅ (ครบทั้ง 4 ไฟล์ตั้งแต่ 3 ส.ค.)
4. เข้า `/staff/company-profile` กรอกข้อมูลบริษัท

---

## รอบเพิ่มเติม (12 ส.ค. 2569)

### ยุบเมนู "พิมพ์และส่งออก" เข้าหน้าเอกสาร

`/staff/print` กับ `/staff/documents` ทำงานเดียวกัน (เอาข้อมูลออกเป็นแผ่น) ต่างกันแค่ขนาดกระดาษ
ซึ่ง `printProfiles.js` ก็เก็บ `label_50x30` ไว้กองเดียวกับ A4 อยู่แล้ว — แยกเมนูไว้ทำให้ผู้ใช้
ต้องจำเองว่าอะไรอยู่ไหน

- `PrintExport.jsx` → `docs/Labels.jsx` เส้นทางใหม่ `/staff/documents/labels`
- `/staff/print` เหลือเป็น redirect (ลิงก์/บุ๊กมาร์กเดิมไม่พัง)
- เมนูบนแดชบอร์ดเหลือ "เอกสาร" อันเดียว

### เอกสารหน้างาน 3 ใบใหม่

ทั้งสามใบใช้ข้อมูลที่มีอยู่แล้ว ไม่ต้อง migration

| ใบ | ไฟล์ | กระดาษ | ข้อมูล |
|---|---|---|---|
| โปสเตอร์ QR เข้าร่วมทริป | `docs/JoinPoster.jsx` | A4 ตั้ง | `tours.join_code` |
| ใบเซ็นชื่อ | `docs/SignatureSheet.jsx` | A4 นอน | `guests` + `buses` + `bus_seats` |
| ป้ายชื่อคล้องคอ | `docs/NameTag.jsx` | A4 ตั้ง 4 ใบ/แผ่น | `guests.qr_token` |

> **ข้อยกเว้นที่จงใจ:** โปสเตอร์ QR เป็นเอกสารใบเดียวที่แสดง `join_code` บนกระดาษ
> เอกสารใบอื่นห้ามแสดง (ดูคอมเมนต์ใน `DocumentHeader.jsx`) เพราะใบนั้นส่งให้คู่ค้า
> ส่วนใบนี้มีไว้ให้ลูกทัวร์อ่านโดยเฉพาะ

### ที่ยังไม่ได้ทำ (คุยไว้แล้ว)

- ชุดเอกสาร (พิมพ์หลายใบต่อกันในครั้งเดียว)
- ปุ่มส่งออก .xlsx ตามชุดคอลัมน์ที่เลือก (`exportXlsx.js` พร้อมแล้ว ขาดแค่ปุ่ม)
- ลิงก์แชร์เอกสารแบบอ่านอย่างเดียว — ต้องออกแบบ token/วันหมดอายุ/ข้อมูลอ่อนไหวก่อน
- นำเข้ารายชื่อจาก CSV — คนละทิศทาง ควรไปอยู่หน้าจัดการลูกทัวร์ ไม่ใช่เมนูเอกสาร
